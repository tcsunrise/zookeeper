...
no relative reference.（只有绝对路径，没有相对路径） Any unicode character can be used in a path subject
...
- The "." character can be used as part of another name, but "."
  ...
  because ZooKeeper doesn't use relative paths（不使用相对路径）. The following would be
  invalid: "/a/b/./c" or "/a/b/../c".
- The token "zookeeper" is reserved.

## ZNodes

> Watches、Data Access、Ephemeral Nodes
...
atomically.(每个节点数据的读写都是原子的？) Reads get all the data bytes associated with a znode and a
