...
(客户端连接服务器的时候，验证信息和链接关联起来)
(在ACL中查找某个客户端的条目，从连接中获取客户端的认证信息)
...
> org.apache.zookeeper.ClientCnxn.AUTHPACKET_XID
> org/apache/zookeeper/ClientCnxn.java:1686
> 这个方法将在客户端发送认证包时调用，zk服务会找到对应方案的插件，并将认证信息和链接关联
