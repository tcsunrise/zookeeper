...
> 客户端获得服务句柄后，会得到一个服务端session，并且还会获得一个密码，用于重连zk服务
...
> Session 超时时间的配置，客户端可以指定超时时间。
> 最小是2倍的tick，最大20倍tick
...
> 断开重连
> 1. 会话未超时，connected；2. 会话已经超时，expired
...
1. handle 句柄 ，开始处于CONNECTING ，连接上一台服务器之后，为 CONNECTED，最终处于 CLOSED
2. 连接串 、超时时间、监视器 Watchers
3. 两层会话模型：本地会话、全局会话；本地可以升级全局会话
4. 会话的创建是全局的，由主节点管理，需要多数节点会话创建成功？
5. 会话是逻辑上的，和一台服务器断开之后，连接到集群中其它机器，会话会继续。
6. 会话的超时时间是2~20倍的tickTime
7. 客户端需要连接服务器发送心跳，以告知服务器其仍在活跃，同时，其连接的节点会通知主节点，会话仍然有效
8. 会话是由服务端管理的
9. 客户端可以获得sessionId和pwd
10. 本地事务可以升级全局事务，可以禁止升级
